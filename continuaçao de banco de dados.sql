...
explain SELECT * 
FROM cliente 
WHERE email LIKE '%souza%';

CREATE INDEX index__cliente_email
ON cliente(email);
...
ALTER TABLE tenis
ALTER COLUMN tamanho TYPE varchar(10) USING tamanho::varchar;


CREATE USER rafaella WITH PASSWORD 'rafinha123';
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO rafaella;

CREATE USER Lara WITH PASSWORD 'amo a vieira';
GRANT SELECT ON cliente TO Lara;
--Com o usuário 'Lara', eu consegui todos os select, porém, o comando 'alter table', apareceu a mensagem que 'é necessário ser o dono da tabela cliente e da tabela tênis'.
...
-- No update de cliente, por exemplo, no inner join apenas desapareceu o usuário e as ruas. Já no left join, desapareceu apenas no endereço, mas o nome do usuário continuou. E por fim, o right join, o nome do usuário aparece de forma nula.